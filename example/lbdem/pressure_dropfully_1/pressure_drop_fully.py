...
# 边界条件设置
# ============================================================
def setContainer(lattice: PSCLattice3D, uwLU):
...
logSteps = max(1, round(0.05 / dtLBM))
subCycles = 1
...
# ============================================================
print('=' * 62)
print(f'  多孔介质压降模拟（全解析）| ε_p = {target_vf:.2f}')
...
contact_model = LinearContactConfig(
    stiffness_normal=4.5e3,
    stiffness_tangential=2.5e3,
    damping_normal=0.3,
...
# ============================================================
log('初始化 LBM 格子 ...')
lattice = PSCLattice3D(Nx, Ny, Nz, omega, dx, dtLBM, rho_f, demsolver)
